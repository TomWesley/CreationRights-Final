// Fetch artists data
 ...
 const ArtistsList = () => {
   ...
   const { isLoading, setIsLoading, currentUser, setActiveView, activeView } = appContext;
   const API_URL = (process.env.REACT_APP_API_URL || 'http://localhost:3001').replace(/\/+$/, '');
   
   // Debugging: Log available context information
   console.log("AppContext in ArtistsList:", {
     availableProperties: Object.keys(appContext),
     ...
     const fetchArtists = async () => {
       ...
       try {
         ...
         const transformedData = data.map(user => ({
           ...
           // Misc
           publicCreations: user.publicCreations || 0,
           featured: user.featured || Math.random() > 0.7 // Randomly mark some as featured for demo
         }));
         
         console.log("Transformed artists data:", transformedData);
   ...
   // New function to start a chat with an artist and navigate to chat page
   // New function to start a chat with an artist and navigate to chat page
const handleStartChat = async (artist) => {
    // Check if the user is logged in
    if (!currentUser) {
      alert('Please log in to start a chat with this artist.');
      return;
    }
    
    // Prevent starting a chat with yourself
    if (artist.email === currentUser.email) {
    ...
    try {
      ...
      // First check if chat already exists by fetching user's chats
      const chatsResponse = await fetch(`${API_URL}/api/chats/${currentUser.email}`);
      if (!chatsResponse.ok) {
        throw new Error('Failed to fetch existing chats');
      }
      
      const existingChats = await chatsResponse.json();
      console.log("Fetched existing chats:", existingChats.length);
      
      // Check if chat with this artist already exists
      const existingChat = existingChats.find(chat => 
        chat.participants && chat.participants.some(p => 
          p.email === artist.email || p.email.toLowerCase() === artist.email.toLowerCase()
        )
      );
      ...
      if (existingChat) {
        // Use existing chat
        console.log("Found existing chat with this artist:", existingChat.id);
        chatId = existingChat.id;
      } else {
        // Create a new chat with the artist
        console.log("Creating new chat with artist:", artist.name);
        const response = await fetch(`${API_URL}/api/chats`, {
          ...
          body: JSON.stringify({
            participants: [
              ...
              { email: artist.email, name: artist.name || artist.email }
            ]
          })
        });
      ...
      if (!chatId) {
        throw new Error('Failed to get valid chat ID');
      }
      
      // Store the chat ID in sessionStorage for the chat page to use
      sessionStorage.setItem('activeChat', chatId);
      
      // Try different possible view names
      const possibleViewNames = ['messages', 'message', 'chat', 'inbox', 'messaging'];
      console.log(`Navigating to messages with chatId: ${chatId}`);
      
      // Set a URL parameter with the chat ID and navigate
      window.history.pushState({chatId}, '', `?chatId=${chatId}`);
      
      // Navigate to chat page
      setActiveView(possibleViewNames[0]);
             ...
             <Card key={artist.id} className={artist.featured ? 'border-2 border-blue-500' : ''}>
                   ...
                   <div className="flex-grow">
                     ...
                     <div className="flex items-center gap-2 mt-1 text-xs text-gray-500">
                       ...
                       <span className="flex items-center">
                         ...
                         Joined {new Date(artist.createdAt || artist.joinedDate).toLocaleDateString()}
                       </span>
                     </div>
                   </div>
                 </div>
               ...
               <CardFooter className="pt-2 flex justify-between">
                 <Button 
                   ...
                   onClick={() => handleStartChat(artist)}
                   disabled={artist.email === currentUser?.email}
                 >
